            ...
            if (playPromise !== undefined) {
                playPromise.catch(error => {
                    ...
                    if (!playBtn) {
                        ...
                        playBtn.textContent = "Tap to Unmute & Play Video";
...
function setupSocket() {
    ...
    socket.on("settings_update", (data) => {
        if (data && data.settings) {
            ...
            console.log("[Socket] Updated proctoring settings received:", window.proctoringSettings);
        }
    });
    ...
    socket.on("browser_stats_update", (data) => {
        if (userRole === "host" || userRole === "interviewer") {
            ...
            if (tabEl) {
                tabEl.textContent = data.tabSwitchCount.toString();
                tabEl.className = data.tabSwitchCount > 0 ? "danger-text" : "gray-text";
            }
            if (copyEl) {
                copyEl.textContent = data.copyPasteCount.toString();
    ...
    socket.on("network_stats_update", (data) => {
        if (userRole === "host" || userRole === "interviewer") {
            ...
            const vpnEl = document.getElementById("stat-network-vpn");
            if (ispEl) {
                ispEl.textContent = `${data.isp} (${data.location})`;
                ispEl.className = "white-text";
            }
            if (vpnEl) {
...
async function startWebcam() {
    try {
        ...
        // Show local video in the main video area until someone else joins
